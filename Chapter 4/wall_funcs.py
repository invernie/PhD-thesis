...
def templateOnly(seed,q,R2,R1 = 18,tau = 0.025, stones = 3000, T = 5000, tint = 100, printing = "off", recordStoneMovement = 'off', ind_var = 'off', var_sd = 0, corners = 'off', showIm = "off"):
...
def templateOnly_patrilines(seed,Rmean,Rsd, f1 = 0.25, f2 = 0.25, f3 = 0.25, stones = 3000, T = 5000, tint = 100, spatSpec = 'off', printing = "off", showIm = "off"):
    ...
    #creates colony population
    R1, R2, R3, R4 = np.random.normal(Rmean, Rsd, 4)
    f1i = int(round(N*f1))
    f2i = int(round(N*f2)) + f1i
    f3i = int(round(N*f3)) + f1i + f2i
    ant_mat = np.repeat(R4,N)
    ant_mat [:f1i] = R1
    ant_mat [f1i+1:f2i] = R2
    ant_mat [f2i+1:f3i] = R3
    ...
    outObj = [Ft, rbar, Ft_list,rbar_list,CoV_list,Rbar_list,space_matrix, [R1,R2,R3,R4]]
...
def rugatulus_delayed(seed,q,R2, Pmax = 0.35, Fmin = 0.01, Fmax = 0.55, Dmax = 0.5, Gmin = 0.01, Gmax = 0.55, tau = 0.025, stones = 3000, printing = "off", delayedPopInc = "on", lateRopt = 24, showIm = "off"):
...
def rugatulus_patrilines(seed,Rmean,Rsd, f1 = 0.25, f2 = 0.25, f3 = 0.25, Pmax = 0.35, Fmin = 0.01, Fmax = 0.55, Dmax = 0.5, Gmin = 0.01, Gmax = 0.55, tau = 0.025, stones = 3000, T = 5000, tint = 100, spatSpec = 'off', printing = "off", showIm = "off"):
...
def bellCurve(seed,q,R2, Pmax = 0.35, Fmin = 0.01, Fmax = 0.55, Dmax = 0.5, Gmin = 0.01, Gmax = 0.55, tau = 0.025, stones = 3000, T = 5000, printing = "off", delayedPopInc = "off", lateRopt = 24, showIm = "off"):
    ...
    def calcDr(rd,Ropt,Sd,Sc):
        ...
        return(Dr)
    def calcPr(rp,Ropt,Sp,Sc):
        Pr = Pmax*(1-(1/(1+tau*(rp-Ropt)**2)))*(Fmin+(Fmax-Fmin)*(1-xp(-(Sc-Sp)**2/(2*sigma**2))))
        return(Pr)
    ...
    while t<T:
        ...
        for n in range(N):
            #at each cycle, each ant has one chance to pick up a pellet; if it picks up, it then repeats its action until the pellet is deposited
            Ropt = ant_mat[n]
            if delayedPopInc == "on" and t>=round(0.5*T):  # for sims where we check for the effect of a delayed increase in worker pop size
                Ropt = lateRopt
            pickedup = 0
            deposited = 0
            while not pickedup:
